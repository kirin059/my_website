...
// 퀴즈 정보 객체
 ...
 // 정답 확인 메서드
 Quiz.prototype.correctAnswer = function(answer) {      //Quiz의 원형(prototype)의 correctAnswer를 불러와서 함수 정의해준다
   return answer == this.questions[this.questionIndex].answer;  //this가 가리키는 함수의 문제, 문제번호, 정답 값이 answer로 리턴
}


// 문제 객체(생성자 함수)
 function Question(text, choice, answer) {         //Question의 원형과 그 안에 객체의 프로토타입 만들기
...
// Question 객체의 생성자 'new Question' 생성자 생성하기(코드의 재사용성)
 ...
 // 입력 및 정답 확인 함수
 function checkAnswer(i) {           //checkAnswer함수에 btn[i]와 이벤트를 정의함으로서, 클릭하면 정답(answer)는 체크한 버튼(btn)이 된다고 함수 정의
    btn[i].addEventListener('click', function() {
       var answer = btn[i].innerText;
 
       if (quiz.correctAnswer(answer)) {   //클릭한 btn이 정답(correctAnswer)일 경우,
          alert('정답입니다!');
          quiz.score++;
       } else {
          alert('틀렸습니다!');
       ...
       if (quiz.questionIndex < quiz.questions.length - 1) {   //전체 퀴즈 길이는 7개인데, 그럼 마지막 문제는 어떻게?
          quiz.questionIndex++;
          updateQuiz();
       } else {
          result();
 ...
 function result() {
    ...
    if (per < 40) {
       txt += '<h2>더 분발하세요</h2>';
       quizDiv.innerHTML = txt;
    } else if (per >= 40 && per < 70 ) {     // &&은 and라는 뜻. 양쪽이 모두 충족해야 true. 반대는 ||(or이라는 뜻. 한쪽만 충족해도 true)
       txt += '<h2>무난한 점수네요</h2>'
       quizDiv.innerHTML = txt;
    } else if (per >= 70) {
       txt += '<h2>훌륭합니다</h2>'   
